...
class IptablesManagerProPlugin(Star):
    ...
    async def set_host(self, event: AstrMessageEvent, host_str: str, username: str, password: str):
        """设置转发主机信息，并自动检查和启用IP转发。用法: /端口映射 设置主机 <ip[:端口]> <用户名> <密码>"""
        async with self.lock:
            if ":" in host_str:
        ...
        try:
            ...
            check_result = await self._run_ssh_command("sysctl net.ipv4.ip_forward")
            
            if "net.ipv4.ip_forward = 1" in check_result:
                yield event.plain_result("IP转发已开启，无需操作。")
            else:
                yield event.plain_result("IP转发未开启，正在尝试永久启用...")
                # 使用独立的配置文件以确保安全
                enable_command = (
                ...
                yield event.plain_result(f"IP转发启用成功。\n{enable_result}")
        except Exception as e:
            yield event.plain_result(f"检查或启用IP转发失败: {e}\n请确保用户有sudo权限，并且SSH连接正常。")
